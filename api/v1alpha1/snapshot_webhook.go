...
func (r *Snapshot) ValidateCreate() (warnings admission.Warnings, err error) {
...
// ValidateUpdate implements webhook.Validator so a (warnings admission.Warnings, err error)  will be registered for the type
func (r *Snapshot) ValidateUpdate(old runtime.Object) (warnings admission.Warnings, err error) {
	snapshotlog := snapshotlog.WithValues("controllerKind", "Snapshot").WithValues("name", r.Name).WithValues("namespace", r.Namespace)
...
func (r *Snapshot) ValidateDelete() (warnings admission.Warnings, err error) {
	snapshotlog := snapshotlog.WithValues("controllerKind", "Snapshot").WithValues("name", r.Name).WithValues("namespace", r.Namespace)
